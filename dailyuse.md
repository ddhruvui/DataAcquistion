...
One-time: `cp runpod/.env.example runpod/.env` and fill it in
...
`config/tickers.json` (EODHD), `config/sharadar.json` (Sharadar), or
...
[README.md](README.md#data-only-watchlist-configwatchlist_vendorjson).
...
scripts/launch.sh all        # DAILY ROUTINE: EODHD + Sharadar + Tiingo (one pod
                                              # each; already-running vendors are skipped, not doubled)
scripts/launch.sh            # EODHD only (default)
scripts/launch.sh nasdaq     # Sharadar / Nasdaq Data Link only
scripts/launch.sh tiingo     # Tiingo only (cold pass ~5 h paced; warm daily runs
...
scripts/clear_storage.sh
#    Logs only (leave data + code intact):
scripts/clear_storage.sh --logs        # add -y to skip confirm
...
scripts/reap_pods.sh              # one pass: report all pods, reap the finished
scripts/reap_pods.sh --dry-run    # report only, delete nothing
scripts/reap_pods.sh --watch      # keep polling
scripts/reap_pods.sh --force <id> # delete one pod without reading its log
...
drive each download. See [README.md](README.md) for the full
